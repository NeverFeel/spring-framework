...
public abstract class HttpServletBean extends HttpServlet implements EnvironmentCapable, EnvironmentAware {
	...
	public ConfigurableEnvironment getEnvironment() {
		//获取environment，如果environment为null，
		// 则new  StandardServletEnvironment()实例
	...
	public final void init() throws ServletException {
		...
		if (!pvs.isEmpty()) {
			try {
				...
				//将ServletContext对象包装成ServletContextResourceLoader对象。
		...
		// 子类来实现，实现自定义的初始化逻辑。目前，FrameworkServlet有具体的代码实现。
		initServletBean();
	}
	...
	private static class ServletConfigPropertyValues extends MutablePropertyValues {
		...
		public ServletConfigPropertyValues(ServletConfig config, Set<String> requiredProperties)
			...
			//这样做的目的是保证我们所需的属性（requiredProperties）都包含在ServletConfig中，如果不是，那么就会抛出
			//ServletException异常信息
			...
			while (paramNames.hasMoreElements()) {
				...
				// 将添加到ServletConfigPropertyValues中的属性从 missingProps 中移除

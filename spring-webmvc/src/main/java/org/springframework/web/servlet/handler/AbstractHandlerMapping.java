...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 */
	/**
	 * 针对当前的handler mapping设置默认的handler。
	 * 如果没有发现特定的mapping，该handler将会被返回。
	 * 默认是null，表示没有默认的handler.
	 */
	 ...
	 */
	/**
	 * 针对当前的handler mapping返回默认的handler，如果不存在则返回null
	 * @return
	 */
	@Nullable
	 ...
	 */
	/**
	 * 通过当前的handler mapping设置应用于所有的handler映射的interceptors。
	 * 支持HandlerInterceptor、WebRequestInterceptor、MappedInterceptor拦截器类型。
	 * 映射interceptors仅仅应用于匹配它的路径模式的请求URLs。
	 * 映射拦截器beans在初始化期间通过类型来检测。
	 * @param interceptors array of handler interceptors
	 * @see #adaptInterceptor
	 * @see org.springframework.web.servlet.HandlerInterceptor
	 * @see org.springframework.web.context.request.WebRequestInterceptor
	 ...
	 */
	/**
	 * 设置基于URL模式的“全局”的CORS配置。
	 * 默认情况下，针对当前的handler首先URL模式匹配会与CORS配置结合在一起(如果需要的话)
	 * @since 4.2
	 * @see #setCorsConfigurationSource(CorsConfigurationSource)
	 */
	 ...
	 */
	/**
	 * 扩展钩子（子类可以重写改方法来注册额外的拦截器），配置拦截器(参见{@link #setInterceptors})。
	 * 在调用方法{@link #initInterceptors()}将指定的interceptors加入到{@link HandlerInterceptor}实例之前，改方法会被调用
	 * 默认是一个空的实现
	 * @param interceptors the configured interceptor List (never {@code null}), allowing
	 * to add further interceptors before as well as after the existing interceptors
	 */
	 ...
	 */
	/**
	 * 检测类型为{@link MappedInterceptor}的bean并且将他们添加到映射拦截器的列表中。
	 * @param mappedInterceptors an empty list to add {@link MappedInterceptor} instances to
	 */
